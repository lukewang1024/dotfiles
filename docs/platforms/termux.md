...
Run the normal bootstrap from the dotfiles checkout:
...
`$XDG_CONFIG_HOME/distributed-workbench/termux-peer.conf` with mode `0600`.
Neither value is committed to dotfiles.

The bootstrap installs the Android release of distributed-workbench, configures
...
cp config/termux/termux-peer.conf.example \
  "${XDG_CONFIG_HOME:-$HOME/.config}/distributed-workbench/termux-peer.conf"
chmod 600 "${XDG_CONFIG_HOME:-$HOME/.config}/distributed-workbench/termux-peer.conf"
```
